...
import { getSongsFolder, getSetlistsFolder, driveRequest, batchDeleteFiles, batchUploadFiles, generateSetlistFilename, generateChordProFilename } from './drive-api.js';
...
export class DriveSyncManager {
    ...
    async sync(progressCallback = null) {
        ...
        try {
            ...
            if (progressCallback) progressCallback({ stage: 'pulling', message: 'Downloading from Drive...' });
    ...
    async clearAndReupload(progressCallback = null) {
        ...
        try {
            ...
            if (progressCallback) progressCallback({ stage: 'uploading', message: 'Uploading to Drive...' });
            await this.pushToDrive(progressCallback);
    ...
    async deleteFolderContents(folderId) {
        ...
        const result = await driveRequest(`/files?q=${encodeURIComponent(query)}&spaces=drive&fields=files(id,name,mimeType)`);
        ...
        for (let i = 0; i < fileIds.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                console.log(`[DriveSync] Batch deleted ${deleted} files (${totalDeleted}/${fileIds.length})`);
            } catch (error) {
    ...
    async buildDriveInventory() {
        ...
        try {
            ...
            const result = await driveRequest(`/files?q=${encodeURIComponent(query)}&spaces=drive&fields=files(id,name,mimeType)&pageSize=1000`);
    ...
    async setlistNeedsSync(setlist) {
        ...
        if (!this.fileExistsInDrive(setlist.driveFileId)) {
            console.log(`[DriveSync] Setlist ${setlist.id} missing from Drive (driveFileId: ${setlist.driveFileId}) - forcing re-upload`);
            return true;
        }
    ...
    async songVersionNeedsSync(song, version, chordproFile) {
        ...
        if (!this.fileExistsInDrive(version.driveChordproFileId)) {
            console.log(`[DriveSync] Song ${song.id}/${version.id} missing from Drive (driveFileId: ${version.driveChordproFileId}) - forcing re-upload`);
            return true;
        }
    ...
    async pullFromDrive(progressCallback = null) {
        ...
        // Pull setlists
        if (progressCallback) progressCallback({ stage: 'pulling', message: 'Downloading setlists...' });
        ...
        for (const driveFile of driveSetlists) {
            try {
                ...
                if (!localSetlist) {
                    ...
                    const lastSynced = localSetlist.lastSyncedAt ? new Date(localSetlist.lastSyncedAt) : new Date(0);
    ...
    async pushToDrive(progressCallback = null) {
        ...
        if (progressCallback) progressCallback({ stage: 'scanning', message: 'Checking Drive files...' });
        ...
        if (updatedSetlists.length > 0) {
            for (let i = 0; i < updatedSetlists.length; i += this.CONCURRENT_LIMIT) {
                ...
                const results = await Promise.allSettled(
                    batch.map(setlist => this.pushSetlist(setlist))
                );
    ...
    async pushSongs(progressCallback = null) {
        ...
        for (const song of songs) {
            ...
            for (const version of song.versions) {
                ...
                if (await this.songVersionNeedsSync(song, version, chordproFile)) {
                    // Check if file actually exists in Drive
                    const existsInDrive = version.driveChordproFileId && this.fileExistsInDrive(version.driveChordproFileId);
        ...
        console.log(`[DriveSync] ${songsToSync.length} songs need upload, ${updatesToProcess.length} versions need updates`);
                ...
                for (const { song, versionsToSync } of batch) {
                    for (const { version, chordproFile } of versionsToSync) {
                        try {
                            ...
                            console.error(`[DriveSync] Failed to upload song version ${song.id}/${version.id}:`, err);
                        }
                    }
                }
            }
    ...
    async pushSongVersion(song, version, chordproFile) {
        try {
            ...
            if (!version.driveChordproFileId) {
                ...
                await DriveAPI.updateChordProFile(
                    version.driveChordproFileId,
                    chordproFile.content,
                    {
                        contentHash: chordproFile.contentHash,
                        ccliNumber: ccliNumber,
