...
export async function batchUploadFiles(files) {
    ...
    console.log(`[DriveAPI] Uploading ${files.length} files with ${CONCURRENT_LIMIT} concurrent requests...`);
...
export async function findOrCreateRootFolder() {
    ...
    const result = await driveRequest(`/files?q=${encodeURIComponent(query)}&spaces=drive&fields=files(id,name)`);
...
export async function findOrCreateOrganisationFolder(organisationName, organisationId) {
    ...
    const result = await driveRequest(`/files?q=${encodeURIComponent(query)}&spaces=drive&fields=files(id,name,appProperties)`);
...
export async function findOrCreateSongFolder(parentId, songId, folderName, properties = {}) {
    ...
    const resultByProps = await driveRequest(`/files?q=${encodeURIComponent(queryByProps)}&spaces=drive&fields=files(id,name,appProperties)`);
 ...
 */
export async function uploadChordProFile(organisationFolderId, songId, versionId, title, ccliNumber, versionLabel, content, metadata = {}) {
 ...
 */
export async function findChordProFile(organisationFolderId, songId, versionId, title = null, ccliNumber = null) {
    const songsFolderId = await getSongsFolder(organisationFolderId);
...
export async function listSetlists(organisationFolderId) {
    ...
    const result = await driveRequest(`/files?q=${encodeURIComponent(query)}&spaces=drive&fields=files(id,name,appProperties,modifiedTime)&orderBy=modifiedTime desc`);
...
export async function listOrganisations() {
    ...
    const result = await driveRequest(`/files?q=${encodeURIComponent(query)}&spaces=drive&fields=files(id,name,appProperties,createdTime)`);

    // Filter to only Setalight organisation folders
    const organisations = (result.files || []).filter(folder =>
        folder.appProperties?.setalightType === 'organisation'
    );
