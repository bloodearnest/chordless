...
async function handleOAuthCallback(request, env) {
	...
	if (!refresh_token) {
		return jsonResponse({ error: 'No refresh token received. User may need to revoke and re-authorize.' }, 400);
	}
...
async function handleShareSetlist(request, env) {
	...
	await env.SETLISTS.put(
		id,
		JSON.stringify({ setlist, created_at: new Date().toISOString() }),
		{ expirationTtl }
	);
