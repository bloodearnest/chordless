            ...
            <div class="storage-section">
                ...
                ` : this.isAuthenticated ? html`
                    <div class="auth-status authenticated">
                        ...
                        ${this.userInfo ? html`
                            <div class="user-card">
                                ${this.userInfo.avatarDataUrl || this.userInfo.picture ? html`
                                    <img
                                        class="user-avatar"
                                    ...
                                    >
                                ` : html`
                                    <div class="user-avatar">${(this.userInfo.name || this.userInfo.email || '?').charAt(0).toUpperCase()}</div>
                                `}
                                <div class="user-details">
                                    <span class="user-name">${this.userInfo.name || 'Signed in user'}</span>
                                    ${this.userInfo.email ? html`<span class="user-email">${this.userInfo.email}</span>` : ''}
                                </div>
                            </div>
                        ` : ''}
                    </div>
                    ...
                    <p>
                        Cloud features are enabled for this device. Your setlists and songs can be synced to Google Drive.
                    </p>
                ...
                ` : html`
                    ...
                    <p style="opacity: 0.6; font-size: 0.8rem;">
                        <strong>Note:</strong> Authorization happens locally on your device.
    ...
    _renderLocalSummarySection() {
        return html`
            <div class="storage-section">
                <h3>📊 Local Data</h3>
                <p>
                    Overview of the setlists, songs, and cached files stored in this browser. Useful for estimating
                    offline storage impact before clearing data.
                </p>
    ...
    _renderSyncSection(disabled = false) {
        return html`
            <div class="storage-section ${disabled ? 'is-disabled' : ''}">
                ...
                <p>
                    Sync your setlists and songs with Google Drive. Your data will be stored in the "Setalight" folder.
                </p>
                <div class="sync-body">
                    <drive-sync-panel ?disabled=${disabled}></drive-sync-panel>
                    ${disabled ? html`
                        <p class="sync-hint">
                            Connect with Google above to enable syncing features.
                        </p>
                    ` : ''}
                </div>
            </div>
    ...
    _renderDangerSection() {
        return html`
            <div class="storage-section" style="border: 1px solid rgba(231, 76, 60, 0.4);">
                <h3>⚠️ Danger Zone</h3>
                <p>
                    Clear all data from IndexedDB and localStorage for this browser. This action cannot be undone!
                </p>
    ...
    async _testTokenRefresh() {
        try {
            ...
            if (newExpiry > new Date()) {
                alert('✅ Token refresh test passed!\n\n' +
