...
export class DriveSyncPanel extends LitElement {
    ...
    async handleClearAndReupload() {
        ...
        if (!confirm('⚠️ This will DELETE all files in your Google Drive Setalight folder and re-upload everything with the new file structure.\n\nThis action cannot be undone!\n\nAre you sure?')) {
            return;
        }
        ...
        return html`
            <div class="sync-panel">
                ...
                ${this.lastSyncTime ? html`
                    <div class="last-sync">
                        Last synced: ${this.formatSyncTime(this.lastSyncTime)}
                    </div>
                ` : ''}
            </div>
        `;
